...
function Icons:GetAnchor(unitID, defaultAnchor, noUIParent)
    if unitID == "player" and E.db.ElvUIPvP.DrTracker.unitFrames.player.usePersonalNameplate then
    ...
    for i = 1, #anchors do
        ...
        local frame
        if not noUIParent and E.db.ElvUIPvP.DrTracker.unitFrames[unit].anchorUIParent then
            frame = UIParent
        else
            frame = _G[name]
        end
...
do
    ...
    function Icons:AnchorPartyFrames(members)
        local cfg = E.db.ElvUIPvP.DrTracker.unitFrames.party
...
do
    ...
    local function MasqueAddFrame(frame)
        frame:SetNormalTexture(E.db.ElvUIPvP.DrTracker.border)
    ...
    local function CreateIcon(unitID, category)
        ...
        local db = E.db.ElvUIPvP.DrTracker
        ...
        if isNew then
            ...
            ctext:SetFont(db.categoryFont.font or ctext:GetFont(), db.categoryFontSize, db.categoryFont.flags)
            ...
            frame.categoryTextSize = db.categoryFontSize
    ...
    function Icons:OnFrameConfigChanged()
        ...
        for _, tbl in pairs(frames) do
            for _, frame in pairs(tbl) do
                ...
                if frame.categoryTextSize and frame.categoryTextSize ~= db.categoryFontSize then
                    -- frame.categoryText:SetFont(db.categoryFont.font or frame.categoryText:GetFont(), db.categoryFontSize, db.categoryFont.flags)
                    frame.categoryText:SetFont(db.categoryFont.font or frame.categoryText:GetFont(), db.categoryFontSize, db.categoryFont.flags)
                    frame.categoryText:SetShown(db.showCategoryText)
                    frame.categoryTextSize = db.categoryFontSize
                end
...
do
    ...
    local function SetIndicators(frame, applied, category)
        ...
        if E.db.ElvUIPvP.DrTracker.timerText and E.db.ElvUIPvP.DrTracker.timerColors then
        ...
        if E.db.ElvUIPvP.DrTracker.colorBlind then
    ...
    local function SetSpellTexture(frame, timer)
        --TODO: ICON Overrider by DR category...
        -- if E.db.ElvUIPvP.DrTracker.categoryTextures[timer.category] then
        --     return frame.icon:SetTexture(E.db.ElvUIPvP.DrTracker.categoryTextures[timer.category])
        -- end
    ...
    function Icons:StartCooldown(timer, unitID, onAuraEnd)
        ...
        if unitID == "player" then
            if E.db.ElvUIPvP.DrTracker.unitFrames.player.usePersonalNameplate then
        ...
        if frame.shown then
            ...
            if not onAuraEnd or E.db.ElvUIPvP.DrTracker.timerStartAuraEnd then
