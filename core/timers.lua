...
function Timers:Insert(unitGUID, srcGUID, category, spellID, isFriendly, isNotPetOrPlayer, isApplied, testMode, ranFromUpdate)
    if isApplied then -- SPELL_AURA_APPLIED
        if E.db.ElvUIPvP.DrTracker.timerStartAuraEnd and not testMode then
        ...
        if not E.db.ElvUIPvP.DrTracker.timerStartAuraEnd then
            if activeTimers[unitGUID] and activeTimers[unitGUID][category] then
    ...
    if ranFromUpdate and not E.db.ElvUIPvP.DrTracker.timerStartAuraEnd then
...
do
    ...
    local function Start(timer, isApplied, unitID, isUpdate, isRefresh, onAuraEnd)
        ...
        -- Check if disabled
        local settings = E.db.ElvUIPvP.DrTracker.unitFrames[gsub(unitID, "%d", "")]
        ...
        if not NS.IS_CLASSIC then
            if isApplied and not E.db.ElvUIPvP.DrTracker.timerStartAuraEnd then
        ...
        for _unit, guid in pairs(activeGUIDs) do
            if guid == unitGUID then
                ...
                if _unit == "player" then
                    if E.db.ElvUIPvP.DrTracker.unitFrames.party.isEnabledForZone then
